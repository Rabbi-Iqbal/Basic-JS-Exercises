...
function protect_email(email) {
  var s = email.split("@");
  var end = (s[0].length) / 2;
  return s[0].substring(0, end) + "....@" + s[1];
}
...
function remove_first_occurrence(text, searchstring) {
  var indexNo = text.indexOf(searchstring);
  return text.slice(0,indexNo) + text.slice(indexNo + searchstring.length + 1);
}
...
function most_frequent(arr) {
  ...
  for(var i=0; i<arr.length; i++){
    ...
    if (counter > noOfOccurence){
      noOfOccurence = counter;
      mostFrequentlyOccured = arr[i];
    }
  }
  console.log("Most frequently occuring item in arr: " + mostFrequentlyOccured);
  console.log(mostFrequentlyOccured + " occurs " + noOfOccurence + " times.")
}
...
function remove_duplicates(arr) {
  ...
  console.log("Duplicates removed from array " + noDuplicates);
}
...
function dash_in_even(number) {
  ...
  for (var i=0; i<temp.length; i++){
    dashingNo.push(temp[i]);
    if((Number(temp[i])%2 == 0) && (Number(temp[i+1])%2 == 0)){
      dashingNo.push("-");
    }
  }
  console.log("even numbers separated by dashes " + dashingNo.join(''));
